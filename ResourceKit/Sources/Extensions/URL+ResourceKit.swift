...
public extension URL {
    ...
    ///
    /// Only a few common filename extensions are defined here.  Over time, as
    /// the number of uunique use cases grows, it is expected that more filename
...
public extension URL {
    ...
    /// `ExtensionType` value of `none` is returned.
...
public extension URL {
    ...
    ///
    /// If the URL's path extension has a defined `ExtensionType` value that
    /// maps to decoder that conforms to the `AnyDecoder` protocol, then a
    /// a decoder appropriate for that path extension is returned.  Otherwise, a
    ...
    /// an object adopting the `Decodable` protocol.
    ///
